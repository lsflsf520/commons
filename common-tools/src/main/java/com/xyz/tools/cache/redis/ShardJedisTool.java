...
import com.xyz.tools.cache.constant.JedisKeyType;
import com.xyz.tools.cache.exception.JedisClientException;
...
//	}
	 ...
	 * @throws JedisClientException
	 */
	public static boolean rpush(JedisKeyNS key, Serializable id, String... members)
			throws JedisClientException {
		...
		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (sadd) only can operate " + JedisKeyType.LIST
							+ ". but found this -> key=" + key.getNameSpace()
							+ " key.keyType=" + key.getKeyType());
		}

		RedisReturn<Long> rs = executor.rpush(key, id, members);
	...
	public static String lpop(JedisKeyNS key, Serializable id) {
		...
		RedisReturn<String> rs = executor.lpop(key, id);
	 ...
	 * @return
	 */
	public static String spop(JedisKeyNS key, Serializable id) {
		if (!JedisKeyType.SET.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (smembers) only can operate "
							+ JedisKeyType.SET + ". but found this -> key="
		...
		RedisReturn<String> rs = executor.spop(key, id);
