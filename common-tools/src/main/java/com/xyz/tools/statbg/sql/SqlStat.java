package com.xyz.tools.statbg.sql;
...
import com.xyz.tools.statbg.util.SqlUtil;
...
abstract public class SqlStat extends Stat {
	...
	protected int maxRetryCount = 5; // 当发生 No operations allowed after
										// statement
	protected String notNullColName;
	...
	public void setNotNullColName(String notNullColName) {
		this.notNullColName = notNullColName;
	}
	...
	public List<FlowData> execute(Map<String, List<FlowData>> paramMap) {
		...
		try {
			...
			for (int i = 0; i < maxLines; i++) {
				if(StringUtils.isNotBlank(notNullColName)){
					Object notNullValue = SqlUtil.getValue(paramMap, notNullColName, i, repeatOneLine, nullReplaceMap);
					
					if(notNullValue == null){
						LOG.warn("found Null value for col " + notNullColName);
						continue;
					}
				}
