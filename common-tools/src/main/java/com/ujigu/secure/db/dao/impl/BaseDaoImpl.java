package com.ujigu.secure.db.dao.impl;
...
import com.ujigu.secure.db.multi.SqlSessionTemplateFactory;
...
//		paramMap.put(MSDSKeyHolder.FORCE_MASTER, forceMaster);
		...
		return findByPage(t, pageInfo, dynamicSql, forceMaster, "getCntByPage", "findByPage");
	}
	
	protected PageImpl<T> findByPage(T t, PageInfo pageInfo, String dynamicSql, boolean forceMaster, String cntStatement, String byPageStatement) {
...
//		paramMap.put(MSDSKeyHolder.FORCE_MASTER, forceMaster);
		...
		List<Integer> count = getSqlSessionTemplate().selectList(namespace + "." + cntStatement, paramMap);
		if (count != null && !count.isEmpty()) {
			pageInfo.setTotalCounts(count.get(0));
		}

		List<T> tList = getSqlSessionTemplate().selectList(namespace + "." + byPageStatement, paramMap);
		if (tList == null || tList.isEmpty()) {
			return new PageImpl<T>(new ArrayList<T>());
		}
...
//		paramMap.put(MSDSKeyHolder.FORCE_MASTER, forceMaster);
	...
	protected String getMapperNamespace() {
		...
		}else if(namespace.endsWith("Repo")){
			namespace = namespace.replace("Repo", "Dao");
		}
