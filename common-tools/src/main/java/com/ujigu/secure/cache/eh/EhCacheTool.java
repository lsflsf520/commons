package com.ujigu.secure.cache.eh;
...
import com.ujigu.secure.cache.constant.CacheConstant;
...
//	private final static CacheManager MANAGER = CacheManager.create("D:/repos/git/stiku_all/stiku-parent/common-parent/common-cache/src/test/resources/ehcache.xml");
	...
	private static EhCacheTool getInstance(){
	...
	public static void put(CacheNameSpace namespace, Serializable key, Object value){
		...
		Element elem = null;
		Object cloneVal = BeanUtils.deepClone(value);
		if(namespace.getExpire() > 0){
			elem = new Element(key, cloneVal, false, namespace.getExpire(), namespace.getExpire());
		}else{
			elem = new Element(key, cloneVal);
		}
		cache.put(elem);
	}
