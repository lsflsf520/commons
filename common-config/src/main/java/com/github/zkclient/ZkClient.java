...
public class ZkClient implements Watcher, IZkClient {
	 ...
	 *            each corresponding to a zk server. e.g.
	 ...
	 *            client would be rooted at "/app/a" and all paths would be
	 *            relative to this root - ie getting/setting/etc... "/foo/bar"
	 *            would result in operations being run on "/app/a/foo/bar" (from
		...
		try {
			...
			if (!waitUntilConnected(maxMsToWaitUntilConnected, TimeUnit.MILLISECONDS)) {
				throw new ZkTimeoutException(String.format(
						"Unable to connect to zookeeper server[%s] within timeout %dms", _connection.getServers(),
