...
public class ShardJedisTool {
	...
	public static final String RedisNull = "nil";
	...
	public static boolean subscribe(final JedisPubSub jedisPubSub, final String... channels){
		boolean result = true;
		try {
			result = executor.subscribeExec(jedisPubSub, channels);
		} catch (JedisConnectionException e) {
			LOGGER.error("executor.subscribe", e);
			result = false;
		}
		return result;
	}
	...
	public static boolean publish(final String channel, final String message){
		int result = 0;
		try {
			result = executor.publish(channel, message);
		} catch (JedisConnectionException e) {
			LOGGER.error("executor.subscribe", e);
			return false;
		}
	 ...
	 * @throws JedisClientException
	 */
	public static boolean hincrby(JedisKeyNS key, Serializable id, String field,long value)
			throws JedisClientException {

		if (!JedisKeyType.HASH.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (hincrby) only can operate "
							+ JedisKeyType.HASH + ". but found this -> key="
							+ key.getNameSpace() + " key.keyType="
							+ key.getKeyType());
		}
		...
		if (!lastResult) {
			throw new JedisClientException("ERROR_500", "exec hincrby: {JedisKeyNS:"
					+ key
					+ ",id:"
					+ id + ",field:" + field + ",value:" + value +"} ");
		}
	 ...
	 * @throws JedisClientException
     */
	public static boolean hdecr(JedisKeyNS key, Serializable id, String field)
			throws JedisClientException {
		return hincrby(key,id,field,-1L);
	}

	/**
	 * 将名称为key的hash中field的value递增
	 * @param key
	 * @param id
	 * @param field
	 * @return
	 * @throws JedisClientException
     */
	public static boolean hincr(JedisKeyNS key, Serializable id, String field)
			throws JedisClientException {
		return hincrby(key,id,field,1L);
	}
	 ...
	 * @throws JedisClientException
	 */
	public static boolean rpush(JedisKeyNS key, Serializable id, Object value)
			throws JedisClientException {

		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (rpush) only can operate "
							+ JedisKeyType.LIST + ". but found this -> key="
							+ key.getNameSpace() + " key.keyType="
							+ key.getKeyType());
		}
		...
		RedisReturn<Long> rs = executor.rpush(key, id, value);

		boolean lastResult = !rs.isAllDown();

		if (!lastResult) {
			throw new JedisClientException("ERROR_500", "exec rpush: {JedisKeyNS:"
					+ key
					+ ",id:"
					+ id + ",value:" + value + "} ");
		}
	 ...
	 * @throws JedisClientException
	 */
	public static boolean lpush(JedisKeyNS key, Serializable id, Object value)
			throws JedisClientException {

		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (lpush) only can operate "
							+ JedisKeyType.LIST + ". but found this -> key="
							+ key.getNameSpace() + " key.keyType="
							+ key.getKeyType());
		}
		...
		RedisReturn<Long> rs = executor.lpush(key, id, value);

		boolean lastResult = !rs.isAllDown();

		if (!lastResult) {
			throw new JedisClientException("ERROR_500", "exec lpush: {JedisKeyNS:"
					+ key
					+ ",id:"
					+ id + ",value:" + value + "} ");
		}
	 ...
	 * @throws JedisClientException
	 */
	public static Long llen(JedisKeyNS key, Serializable id)
			throws JedisClientException {

		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (llen) only can operate " + JedisKeyType.LIST
							+ ". but found this -> key=" + key.getNameSpace()
							+ " key.keyType=" + key.getKeyType());
		}
		...
		try {
			rs = executor.llen(key, id);
		} catch (JedisConnectionException e) {
			LOGGER.error(
					"executor.llen: {JedisKeyNS:" + key + ",id:" + id + "}", e);
		}
	 ...
	 * @throws JedisClientException
	 */
	public static List<String> lrange(JedisKeyNS key, Serializable id, final long start, final long end)
			throws JedisClientException {

		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (lrange) only can operate " + JedisKeyType.LIST
							+ ". but found this -> key=" + key.getNameSpace()
							+ " key.keyType=" + key.getKeyType());
		}

		RedisReturn<List<String>> rs = new RedisReturn<List<String>>();

		try {
			rs = executor.lrange(key, id,start,end);
		} catch (JedisConnectionException e) {
			LOGGER.error(
					"executor.lrange: {JedisKeyNS:" + key + ",id:" + id + "}", e);
		}
	...
	/**
	 * 截断名称为key的list，保留start至end之间的元素（下标从0开始）
	 *
	 * @param key
	 *            跟id参数一起组成Redis里面实际的KEY
	 * @param id
	 ...
	 * @throws JedisClientException
	 */
	public static boolean ltrim(JedisKeyNS key, Serializable id, final long start, final long end)
			throws JedisClientException {

		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (ltrim) only can operate "
							+ JedisKeyType.LIST + ". but found this -> key="
							+ key.getNameSpace() + " key.keyType="
							+ key.getKeyType());
		}
		...
		RedisReturn<String> rs = executor.ltrim(key, id,start,end);

		boolean lastResult = !rs.isAllDown();

		if (!lastResult) {
			throw new JedisClientException("ERROR_500", "exec ltrim: {JedisKeyNS:"
					+ key
					+ ",id:"
	 ...
	 * @throws JedisClientException
	 */
	public static String lindex(JedisKeyNS key, Serializable id, final long index)
			throws JedisClientException {

		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (lindex) only can operate " + JedisKeyType.LIST
							+ ". but found this -> key=" + key.getNameSpace()
							+ " key.keyType=" + key.getKeyType());
		}
		...
		try {
			rs = executor.lindex(key, id,index);
		} catch (JedisConnectionException e) {
			LOGGER.error(
					"executor.lindex: {JedisKeyNS:" + key + ",id:" + id + ",index:" + index +"}", e);
		}
	 ...
	 * @throws JedisClientException
	 */
	public static boolean lset(JedisKeyNS key, Serializable id,long index,Object value)
			throws JedisClientException {

		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (lset) only can operate "
							+ JedisKeyType.LIST + ". but found this -> key="
							+ key.getNameSpace() + " key.keyType="
							+ key.getKeyType());
		}
		...
		RedisReturn<String> rs = executor.lset(key, id,index,value);

		boolean lastResult = !rs.isAllDown();

		if (!lastResult) {
			throw new JedisClientException("ERROR_500", "exec lset: {JedisKeyNS:"
					+ key
					+ ",id:"
					+ id + ",index:" + index +  ",value:" + value+"} ");
		}
	 ...
	 * @throws JedisClientException
	 */
	public static boolean lrem(JedisKeyNS key, Serializable id,long count,Object value)
			throws JedisClientException {

		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (lrem) only can operate "
							+ JedisKeyType.LIST + ". but found this -> key="
							+ key.getNameSpace() + " key.keyType="
							+ key.getKeyType());
		}
		...
		RedisReturn<Long> rs = executor.lrem(key, id,count,value);

		boolean lastResult = !rs.isAllDown();

		if (!lastResult) {
			throw new JedisClientException("ERROR_500", "exec lrem: {JedisKeyNS:"
					+ key
					+ ",id:"
					+ id + ",count:" + count +  ",value:" + value+"} ");
		}
	 ...
	 * @throws JedisClientException
	 */
	public static String lpop(JedisKeyNS key, Serializable id)
			throws JedisClientException {

		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (lpop) only can operate " + JedisKeyType.LIST
							+ ". but found this -> key=" + key.getNameSpace()
							+ " key.keyType=" + key.getKeyType());
		}
		...
		try {
			rs = executor.lpop(key, id);
		} catch (JedisConnectionException e) {
			LOGGER.error(
					"executor.lpop: {JedisKeyNS:" + key + ",id:" + id + "}", e);
		}
	 ...
	 * @throws JedisClientException
	 */
	public static String rpop(JedisKeyNS key, Serializable id)
			throws JedisClientException {

		if (!JedisKeyType.LIST.equals(key.getKeyType())) {
			throw new JedisClientException("404",
					"this method (rpop) only can operate " + JedisKeyType.LIST
							+ ". but found this -> key=" + key.getNameSpace()
							+ " key.keyType=" + key.getKeyType());
		}
		...
		try {
			rs = executor.rpop(key, id);
		} catch (JedisConnectionException e) {
			LOGGER.error(
					"executor.rpop: {JedisKeyNS:" + key + ",id:" + id + "}", e);
		}
	...
	/**
	 * 慎用!!!将一个名称为key的list存入,存入的listObject的头元素在redis将成为尾元素，原有同key的list将被删除，不保证存入的事务性。
	 * @param key
	 * @param id
	 * @param listObject
     * @return
     */
	public static boolean lPutListToHead(JedisKeyNS key, Serializable id,List<Object> listObject){
		try {
			del(key,id);
			...
			LOGGER.error("lPutListToHead Error,will del the list :",e);
			del(key,id);
			return false;
		}
		return true;
	...
	/**
	 * 慎用!!!将一个名称为key的list存入,存入的listObject的头元素在redis还是头元素，原有同key的list将被删除，不保证存入的事务性。
	 * @param key
	 * @param id
	 * @param listObject
	 * @return
	 */
	public static boolean lPutListToTail(JedisKeyNS key, Serializable id,List<Object> listObject){
		try {
			del(key,id);
			for (Object o:listObject){
				rpush(key,id,o);
	...
	/**
	 * 慎用!!!从头开始取出list中的元素,list中的元素将被清除，不保证取出的事务性
	 * @param key
	 * @param id
     * @return
     */
	public static List<String> lTakeListFromHeadToTail(JedisKeyNS key, Serializable id){
		List<String> resultList = new ArrayList<String>();
		String flag = lpop(key,id);
		while(null!=flag&&!flag.equals(RedisNull)){
			resultList.add(flag);
			flag = lpop(key,id);
		}
		return resultList;
	...
	/**
	 * 慎用!!!从头开始取出count个list中的元素,被取出元素将被清除，不保证取出的事务性
	 * @param key
	 * @param id
     * @return
     */
	public static List<String> lTakeListFromHeadToTailCount(JedisKeyNS key, Serializable id,long count){
		List<String> resultList = new ArrayList<String>();
		String flag = lpop(key,id);
		int i=0;
		while(null!=flag&&!flag.equals(RedisNull)&&i<count){
			resultList.add(flag);
			flag = lpop(key,id);
			i++;
		}
	...
	/**
	 * 慎用!!!从尾开始取出list中的元素,list中的元素将被清除，不保证取出的事务性
	 * @param key
	 * @param id
	 * @return
	 */
	public static List<String> lTakeListFromTailToHead(JedisKeyNS key, Serializable id){
		List<String> resultList = new ArrayList<String>();
		String flag = rpop(key,id);
		while(null!=flag&&!flag.equals(RedisNull)){
			resultList.add(flag);
			flag = rpop(key,id);
		}
	...
	/**
	 * 慎用!!!从尾开始取出count个list中的元素,被取出元素将被清除，不保证取出的事务性
	 * @param key
	 * @param id
	 * @return
	 */
	public static List<String> lTakeListFromTailToHeadCount(JedisKeyNS key, Serializable id,long count){
		List<String> resultList = new ArrayList<String>();
		String flag = rpop(key,id);
		int i=0;
		while(null!=flag&&!flag.equals(RedisNull)&&i<count){
