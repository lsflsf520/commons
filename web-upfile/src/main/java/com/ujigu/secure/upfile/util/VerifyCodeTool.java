...
//    private static final String CVCNUMBERS = "0123456789+";
     ...
     * */  
    public BufferedImage drawVerificationCodeImage(int width, int height){  
        //image  
...
//      g.setColor(getRandomColor(200,250));  
        ...
        for(int i =0;i < DISTURB_LINE_SIZE; i++){  
            drawDisturbLine1(g, width, height);  
            drawDisturbLine2(g, width, height);  
        }  
        //Generate a random number, set return data  
        getRandomMathString();  
        LogUtils.debug("验证码 : %s", randomString);  
        LogUtils.debug("验证码结果 : %s", xyresult);  
...
//              suChinese.append(CNUMBERS[yy]);  
     ...
     *@param g Graphics 
     * */  
    public void drawDisturbLine1(Graphics g, int width, int height){  
     ...
     *@param g Graphics 
     * */  
    public void drawDisturbLine2(Graphics g, int width, int height){  
        int x1 = random.nextInt(width);  
        int y1 = random.nextInt(height);  
        int x2 = random.nextInt(13);  
        int y2 = random.nextInt(15);  
